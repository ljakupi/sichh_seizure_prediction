# -*- coding: utf-8 -*-
"""
Created on Wed Feb 13 09:33:20 2019
...
class Config():
  ...
  def __init__(self, data_path="./", patient=1):
    ...
    self.data_path = data_path # Raw data path
    self.patient_folder = data_path+'/Dog_{:01d}/'.format(patient)
    self.output_dir = self.patient_folder+"data/" # Featured data path
    #data parameters
    self.sampling = 399.60980456176128 #Hz, specified by data
...
#                         "FP2-F8","F8-T8","T8-P8-0","P8-O2","FZ-CZ","CZ-PZ"]
    self.channels_names=['LEFT_01', 'LEFT_02', 'LEFT_03', 'LEFT_04',
 'LEFT_05', 'LEFT_06', 'LEFT_07', 'LEFT_08',
 'RIGHT_01', 'RIGHT_02', 'RIGHT_03', 'RIGHT_04',
 'RIGHT_05', 'RIGHT_06', 'RIGHT_07', 'RIGHT_08']
...
#    print('signal: ', signal)
#    print('signal.ch_names: ', signal.ch_names)
...
#    print('size N_segments :', N_segments)
...
#    print('features_len :', self.features_len[feature_name])
#    print('features :', features.shape)
#    print('segments_idx :', self.segments_idx)
#    print('size segments :', self.segments_idx.shape)
...
#      print('time :', time)
#      print('eeg_start:', self.eeg_start)
      
      features[i,0] = time + self.eeg_start
#      print('features:', features)
#      print('size features:', features.shape)
      
#'''single process'''      
#      self.add_features( mp_batch_arr, feature_name, N_segments, time, i)
#      if N_segments % 100 == 0:
#        time.sleep(10)
#      print(i)
#      
...
#    print('size segments', segments.shape)
#    print('time:', time)
#    print('segments[0,1]:', segments[0,1])
    segments[i,1]= time
#    print('segments[0,1:]  ', segments[0,1:])
#    print('size segments[0,1:]  ', segments[0,1:].shape)
...
#    print('size moments:', moments.shape)
...
#    print('features',features)
#    print('size features before',features.shape)
...
#    print('size peak2peak',peak_to_peak.shape)
#    print('size absolute area',absolute_area.shape)
#    print('size psd ratio',psd_ratio.shape)
#    print('size decorrelation',decorrelation_time.shape)
#    print('size dwt coeffs',dwt_coeffs.shape)

    
#    print('size features after',features.shape)
    return features
...
#    print("write_segments")    
  ...
  def load_files(self, cfg):
    files = [f for f in os.listdir(cfg.patient_folder) if os.path.isfile(os.path.join(cfg.patient_folder, f))]
    files = [f for f in files if f.endswith(".edf")]

    files = sorted(files, key=lambda x: int(re.split('\.',re.split('_',x)[3])[0]))
...
#    print(files)
    #[1000:1099]:
    for filename in files[1000:1099]:
      file_number = int(re.search('D001_(.+?).edf', filename).group(1))
      print("Loading: " + filename)
      self.eeg_data[file_number] = self.load_data(self.patient_folder+filename, file_number, cfg)
    
  def load_data(self, filename, file_number, cfg):
    # get info in the summary file
    eeg_start = 3600*file_number
    eeg_end = eeg_start+3600
...
#cfg = Config(data_path ="//sichhnas02.unifr.ch/SICHH SHARE/Facilities/Digital Health/Projects/Novartis Epilepsy/NeuroVista IEEG" , patient = 1)
#cfg = Config(data_path ="C:/Users/BersetYv/Documents/Github/seizure_prediction", patient = 1)
cfg = Config(data_path ="/home/sichhadmin/epilepsy/LSTM", patient = 1)
